...
Analysis carried out using code should meet the same quality standards as any other analysis. That means using the principles and practices set out in the [AQuA Book](https://www.gov.uk/guidance/the-aqua-book) and the [ONS quality standard for analysis](https://officenationalstatistics.sharepoint.com/sites/onswiki/SitePages/Quality_Standards.aspx). 
...
Open source code is a great tool when it comes to building pipeline. There are usually already solutions out there for most common analysis problems that mean you don’t need to write everything from scratch. Both R and python include extensive libraries of code modules that add functionality to the base systems. The [ONS Statistical Methods Library](https://statisticalmethodslibrary.ons.gov.uk/) also includes standard statistical code libraries that are already approved and tested for use at ONS.

When tacking a new problem, check whether a package already exists that can help you. We recommend using packages that are well established and actively maintained, which you can find out by visiting their github repositories or equivalents. However, you must ensure that code you find meets your specifications for the pipeline and make sure that where you re-use code it is appropriately assured.
